use jpk_reader::{self as jpk, ArchiveReader};
use pyo3::{exceptions::PyRuntimeError, prelude::*};
use std::{borrow::Cow, fs, path::PathBuf};

#[pymodule(submodule, name = "qi_map")]
pub mod export {
    #[pymodule_export]
...
pub struct QIMapReader {
    inner: jpk::qi_map::v2_0::FileReader,
}
...
impl QIMapReader {
    ...
    fn new(path: PathBuf) -> PyResult<Self> {
        let file = fs::File::open(&path).map_err(|err| PyRuntimeError::new_err(err.to_string()))?;
        let mut archive =
            zip::ZipArchive::new(file).map_err(|err| PyRuntimeError::new_err(err.to_string()))?;
        let version = jpk::dataset::format_version(&mut archive).map_err(|err| {
            PyRuntimeError::new_err(format!("could not get format version: {err:?}"))
        })?;
        let Some(version) = version else {
            return Err(PyRuntimeError::new_err("unsupported format version"));
        };

        let reader = match version {
            jpk::dataset::FormatVersion::V2_0 => {
                unsafe { jpk::qi_map::v2_0::FileReader::new_with_archive(path, archive) }
            ...
            let components = file
                ...
                .map(|component| {
                    if let Ok(value) = component.parse::<usize>() {
                        Cow::Owned(format!("{:0>width$}", value, width = max_digits as usize))
                    } else {
