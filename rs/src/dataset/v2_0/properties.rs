...
impl Dataset {
    ...
    pub fn file_format_version(&self) -> Option<&String> {
        self.get(crate::dataset::properties::DATASET_FILE_FORMAT_VERSION_KEY)
    }
...
pub mod segment {
    ...
    impl Properties {
        pub fn channel_list(
            &self,
        ) -> Result<Vec<&str>, crate::dataset::error::Error<dataset_properties::error::Property>>
        {
        ...
        ) -> Result<channel::Info, crate::dataset::error::Error<dataset_properties::error::Property>>
        {
            channel::Info::from(self, channel).map_err(|err| crate::dataset::error::Error {
                paths: vec![self.path.clone()],
                error: err,
            })
        }
...
pub mod channel {
    use super::{
        super::{LcdInfoIndexType, SegmentType},
        dataset_properties, segment,
    };
    use std::{fmt, path::PathBuf};
    ...
    impl Info {
        ...
        pub fn from(
            ...
            .map(PathBuf::from)?;
            let file_format = dataset_properties::extract_value!(
                properties,
                segment::Properties::channel_data_file_format_key(&channel),
                from_str DataFileFormat
            )?;
            let num_points = dataset_properties::extract_value!(
                properties,
                segment::Properties::channel_data_num_points_key(&channel),
                parse u32
            )?;
            let lcd_info_index = dataset_properties::extract_value!(
                properties,
                segment::Properties::channel_lcd_info_index_key(&channel),
                parse SegmentType
            )?;
